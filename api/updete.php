<?php 
    ...
    if(!empty($data->id)&&!empty($data->nombre)&&!empty($data->apellidos)&&!empty($data->correo)&&!empty($data->password)){
        if($service->update($data->id,$data->nombre,$data->apellidos,$data->correo,$data->password)){
            http_response_code(201);
            echo json_encode(array("message"=>"Paciente modificado"));
        }else{
